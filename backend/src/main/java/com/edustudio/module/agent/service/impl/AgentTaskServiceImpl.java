...
public class AgentTaskServiceImpl implements AgentTaskService {
    ...
    public AgentTaskResultVO createAndRun(AgentTaskCreateRequest request) {
        ...
        JsonNode enrichedInput = enrichGenerationInput(request);
        ...
        task.setInputParams(JsonUtils.toJson(enrichedInput));
        task.setExecutionStatus("running");
        task.setStartedAt(LocalDateTime.now());
        task.setStatus(ACTIVE_STATUS);
        agentTaskMapper.insert(task);
        if (request.getSpaceId() != null) {
            learningSpaceService.incrementTaskCount(request.getSpaceId());
        }
    ...
    public AgentTaskVO detail(Long id) {
        ...
        List<GeneratedResource> taskResources = generatedResourceMapper.selectList(new LambdaQueryWrapper<GeneratedResource>()
                .eq(GeneratedResource::getTaskId, id)
                .eq(GeneratedResource::getDeleted, 0)
                .orderByDesc(GeneratedResource::getCreatedAt));
        vo.setResources(taskResources.stream().map(generatedResourceService::toVO).toList());
        return vo;
    }
    ...
    private JsonNode enrichGenerationInput(AgentTaskCreateRequest request) {
        ObjectNode params = request.getInputParams() != null && request.getInputParams().isObject()
                ? (ObjectNode) request.getInputParams().deepCopy()
                : (ObjectNode) JsonUtils.fromJson("{}", JsonNode.class);
        List<Long> fileIds = new java.util.ArrayList<>();
        JsonNode sourceIds = params.path("source_file_ids");
        if (sourceIds.isArray()) {
            sourceIds.forEach(value -> {
                if (value.canConvertToLong()) fileIds.add(value.asLong());
            });
        }
        ...
        if (request.getSpaceId() != null && (useKnowledgeBase || !fileIds.isEmpty())) {
            ...
            for (KnowledgeSearchResultVO.Item item : nullSafe(context.getResults())) {
                ObjectNode chunk = contextArray.addObject();
                chunk.put("source", item.getSource());
                if (item.getFileId() != null) chunk.put("file_id", item.getFileId());
                if (item.getChunkIndex() != null) chunk.put("chunk_index", item.getChunkIndex());
                chunk.put("content", item.getChunkText());
            }
            if (!fileIds.isEmpty() && contextArray.isEmpty()) {
                throw new BusinessException(ResultCode.BAD_REQUEST, "所选资料尚未生成可用知识片段，请等待处理完成后再生成资源");
            }
            params.put("grounding_required", !contextArray.isEmpty());
        }
        return params;
    }
